...
class CSDoubleEncoderModel(BaseModel):
    ...
    def loss_function(self, batch: Dict[str, Tensor], output: Dict[str, Tensor], params: Dict[str, Any],
        ...
        if lambda_content_seg > 0:
            b1_content_seg: Tensor = F.log_softmax(self._content_seg_disc(c1_detach), dim=1).gather(dim=1, index=seg1.unsqueeze(1))
            b2_content_seg: Tensor = F.log_softmax(self._content_seg_disc(c2_detach), dim=1).gather(dim=1, index=seg2.unsqueeze(1))
        ...
        if lambda_style_seg > 0:
            b1_style_seg: Tensor = F.log_softmax(self._style_seg_disc(s1_detach), dim=1).gather(dim=1, index=seg1.unsqueeze(1))
            b2_style_seg: Tensor = F.log_softmax(self._style_seg_disc(s2_detach), dim=1).gather(dim=1, index=seg2.unsqueeze(1))
        ...
        if lambda_content_seg > 0:
            b1_content_seg: Tensor = F.log_softmax(self._content_seg_disc(c1), dim=1).gather(dim=1, index=seg1.unsqueeze(1))
            b2_content_seg: Tensor = F.log_softmax(self._content_seg_disc(c2), dim=1).gather(dim=1, index=seg2.unsqueeze(1))
        ...
        if lambda_style_seg > 0:
            b1_style_seg: Tensor = F.log_softmax(self._style_seg_disc(s1), dim=1).gather(dim=1, index=seg1.unsqueeze(1))
            b2_style_seg: Tensor = F.log_softmax(self._style_seg_disc(s2), dim=1).gather(dim=1, index=seg2.unsqueeze(1))
