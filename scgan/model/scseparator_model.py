...
class SCSeparatorBeautyganModel(SCSeparatorModel):
    def __init__(self, device) -> None:
        ...
        self._source_disc: nn.Module = nn.Sequential(
            ...
            nn.Conv2d(32, 64, kernel_size=4, stride=2, padding=1, bias=False), nn.InstanceNorm2d(64, affine=True), nn.LeakyReLU(0.01),
    ...
    def forward(self, batch: Dict[str, Tensor], params: Dict[str, Any],
        ...
        output.update({'b1_source': b1_source, 'b2_source': b2_source, #'b2_source2': b2_source2, #'b1_source2': b1_source2,
                       'b1_reference': b1_reference, 'b2_reference': b2_reference, #'b1_reference2': b1_reference2,
    ...
    def loss_function(self, batch: Dict[str, Tensor], output: Dict[str, Tensor], params: Dict[str, Any],
        ...
        #accuracy_reference: Tensor = ((correct1.sum() + correct12.sum()) / 2. + correct2.sum()
        ...
        loss_content_seg: Tensor = - lambda_content_seg * (
            F.softmax(b1_content_seg, dim=-1).gather(dim=-1, index=seg1.unsqueeze(-1)) +
            F.softmax(b2_content_seg, dim=-1).gather(dim=-1, index=seg1.unsqueeze(-1))) / 2.
        #loss_content_seg: Tensor = lambda_content_seg * (
        ...
        loss_style_seg: Tensor = - lambda_style_seg * (
                F.softmax(b1_style_seg, dim=-1).gather(dim=-1, index=seg1.unsqueeze(-1)) +
                F.softmax(b2_style_seg, dim=-1).gather(dim=-1, index=seg1.unsqueeze(-1))) / 2.
